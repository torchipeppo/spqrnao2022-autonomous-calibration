...
std::tuple<float,int> RoleAndContextProvider::minOpponent_BallDistance(){
    ...
    for(auto obstacle : theTeamPlayersModel.obstacles){
        if(obstacle.type == Obstacle::opponent){
            dist = theLibCheck.distance( Pose2f(obstacle.center), Pose2f(theTeamBallModel.position) );
            if(dist < minDist ){
                minDist = dist;
            }
            if(dist < 1500.f){
...
std::tuple<float,int, int> RoleAndContextProvider::minTeammate_BallDistance(){
    ...
    for(auto teammate : theTeamData.teammates){
        dist = theLibCheck.distance( teammate.theRobotPose, Pose2f(theTeamBallModel.position) );
        if(dist < minDist ){
            minDist = dist;
...
// std::cout<<"rolasdasdasdde hehe   "<<std::endl;
