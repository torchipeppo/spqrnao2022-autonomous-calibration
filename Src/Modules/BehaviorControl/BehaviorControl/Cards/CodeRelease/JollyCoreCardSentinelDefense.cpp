...
CARD(JollyCoreCardSentinelDefense,
{,
  CALLS(Activity),
  CALLS(LookForward),
  CALLS(LookAtPoint),
...
class JollyCoreCardSentinelDefense : public JollyCoreCardSentinelDefenseBase
{
  ...
  bool preconditions() const override
  {
    ...
    bool ready2pass=false;
    //PassShare
    for (const auto &teammate : theTeamData.teammates)
    {
      if (teammate.role == Role::RoleType::striker)
      {
        // std::cout <<"Jolly defense striker setting  ready2pass: " << teammate.thePassShare.readyPass  << std::endl;
        strikerPosition = Vector2f(teammate.theRobotPose.translation.x(), teammate.theRobotPose.translation.y());
        ready2pass=!((bool)teammate.thePassShare.readyPass);
      }
    }
    // std::cout<<"theposREB    " << theLibCheck.rel2Glob(position.x(),position.y()).translation.x() << std::endl;

    // std::cout <<"Jolly defense pos : " << theLibCheck.rel2Glob(position.x(),position.y()).translation.x()  << std::endl;
    //bool isStriker_defender = ready2pass;//theLibCheck.rel2Glob(position.x(),position.y()).translation.x() <0;//strikerPosition.x()<0&&position.x()<0;
    bool isStriker_defender = theLibCheck.rel2Glob(position.x(),position.y()).translation.x() <1000;//strikerPosition.x()<0&&position.x()<0;
    // std::cout <<"Jolly defender : " << isStriker_defender<< std::endl;
    return true;//isStriker_defender;
    //return (theGameInfo.state == STATE_PLAYING  && velocity.x() >= 0 && velocity.norm()/position.norm() >= 0.7);
  }

  bool postconditions() const override
  {
    //this part needs to be modified, ask to the team
    Vector2f velocity = theBallModel.estimate.velocity;
    Vector2f position = theBallModel.estimate.position;
  ...
  option
  {
    theActivitySkill(BehaviorStatus::JollyCoreCardSentinelDefense);
    initial_state(start)
    {
      transition
      {
      ...
      action
      {
        ...
        theWalkToTargetSkill(Pose2f(walkSpeed, walkSpeed, walkSpeed),
                         Pose2f(theLibCheck.glob2Rel(theLibCheck.jollyPosition.x(),
                                                     theLibCheck.jollyPosition.y())));
        theLookForwardSkill();
      }
    }
    ...
    {  
      transition{
               if(std::abs( theLibCheck.radiansToDegree(theLibCheck.angleForJolly) ) < 10.f )
                goto walkToPose;
      }
      ...
      action{
        if(state_time > 500.f){
            theLookForwardSkill();
             ...
             if((theLibCheck.angleForJolly) > 0.f)
 ...
 state(walkToPose)
    {
        transition
        {
//            if(/*(std::abs(theLibCodeRelease.angleForJolly) > TOLERANCE_DEGREES_FOR_TURN) &&*/
//                    (theRobotPose.translation - theLibCodeRelease.JollyPosition).norm() > DO_TURN_THRESHOLD)
//                goto turnToPose;

//            for(int i = 0; i < theTeamData.teammates.size(); i++){
//                if((theTeamData.teammates.at(i).theRobotPose.translation -
//                        theLibCodeRelease.JollyPosition).norm() < (radiusSupp)){
//                    radiusSupp -= 10.f;
//                    goto stand;
//                }
//            }
            ...
            if((theRobotPose.translation - theLibCheck.jollyPosition).norm() < STATIC_THRESHOLD)
                goto wait;
        }
        action
        {
            ...
            theLookLeftAndRightSkill();
            // std::cout<<"optimal jolly pos "<<theLibCheck.jollyPosition.transpose()<<std::endl;
            if((theLibCheck.glob2Rel(theLibCheck.jollyPosition.x(),
                                           theLibCheck.jollyPosition.y())).translation.norm() < 3*STATIC_THRESHOLD){
                
                std::cout<<"def 1 optimal jolly pos "<<theLibCheck.jollyPosition.transpose()<<std::endl;
                        
                theWalkToTargetSkill(Pose2f(50.f, 50.f, 50.f), Pose2f(theLibCheck.glob2Rel(theLibCheck.jollyPosition.x(),
                                                                                     theLibCheck.jollyPosition.y())));
            } else {

                std::cout<<"def 2 optimal jolly pos "<<theLibCheck.jollyPosition.transpose()<<std::endl;
                theWalkToTargetPathPlannerSkill(Pose2f(1.f, 1.0f, 1.0f), Pose2f(theLibCheck.jollyPosition.x(),
                                                              theLibCheck.jollyPosition.y()));
                //Arms back
        ...
        state(stand){
        transition{
//            std::cout<< "I'm the jolly and I am waiting!"<< std::endl;
...
MAKE_CARD(JollyCoreCardSentinelDefense);