...
class JollyCoreCardRebound : public JollyCoreCardReboundBase
{
  ...
  bool preconditions() const override
  {
    ...
    // std::cout<<"theposDEF    "<< theLibCheck.rel2Glob(position.x(),position.y()).translation.x() << std::endl;

    bool isStriker_shooting = theLibCheck.rel2Glob(position.x(),position.y()).translation.x() > 350;//strikerPosition.x()<0&&position.x()<0;
    return isStriker_shooting;
  ...
  option
  {
    theActivitySkill(BehaviorStatus::jollyCoreSentinelDefense);
    initial_state(start)
    {
      transition
      {
 ...
 state(moveAroundObstacle)
    {
            ...
            if((theRobotPose.translation-theLibCheck.getJollyPosition()).norm() < 100.f){
                if(theRoleAndContext.ball_holding_Context == 2 || theRoleAndContext.ball_holding_Context == 3){
                    if(theTeamBallModel.position.x() < 0.f || theLibCheck.opponentOnOurField())
                     {
                            //std::cout<< "jolly turnopponent_from_movearound"<< std::endl;

                            goto turnToOpponent;
                     }
                    else {
                        //std::cout<< "jolly turnToball_from_movearound"<< std::endl;
        ...
        action 
        {
            // std::cout<< "movearound_action  "<<theLibCheck.getJollyPosition().x()<<"    "<<theLibCheck.getJollyPosition().y()<< std::endl;
   ...
   state(move)
    {
        transition
        {
            ...
            if((theRobotPose.translation-theLibCheck.getJollyPosition()).norm() < 100){
               if(theRoleAndContext.ball_holding_Context == 2 || theRoleAndContext.ball_holding_Context == 3){
                    if(theTeamBallModel.position.x() < 0.f || theLibCheck.opponentOnOurField())
                      {
            ...
            Vector2f targetPos = theLibCheck.getJollyPosition();//getSupporterPosition();
        ...
        action 
        {
            ...
            if((theRobotPose.translation-theLibCheck.getJollyPosition()).norm() > JOLLY_WALK_THRESHOLD){
                ...
                // theWalkToTargetPathPlannerSkill(Pose2f(1.f,1.f,1.f),theLibCheck.getJollyPosition());
                theWalkToTargetSkill(Pose2f(1.f,1.f,1.f),theLibCheck.getJollyPosition());
                theLookLeftAndRightSkill();
            }
            else {
    ...
    state(turnToOpponent)
    {
        transition
        {
            ...
            Vector2f interCalcPosition = theLibCheck.getJollyPosition() - strikerPosition;
        ...
        action
        {
            ...
            Vector2f interCalcPosition = theLibCheck.getJollyPosition();
...
MAKE_CARD(JollyCoreCardRebound);