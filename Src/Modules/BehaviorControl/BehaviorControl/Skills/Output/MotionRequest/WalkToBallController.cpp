...
SKILL_IMPLEMENTATION(WalkToBallControllerImpl,
{,
  IMPLEMENTS(WalkToBallController),
  REQUIRES(LibCheck),
  REQUIRES(MotionInfo),
  REQUIRES(FieldBall),
  REQUIRES(PassShare),
...
class WalkToBallControllerImpl : public WalkToBallControllerImplBase
{
  void execute(const WalkToBallController& p) override
  {
  ...
  bool isDone(const WalkToBallController&) const override
  {
    return theMotionInfo.motion == MotionRequest::walk;
  }
};

MAKE_SKILL_IMPLEMENTATION(WalkToBallControllerImpl);
